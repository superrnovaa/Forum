...
func CreateTables() {
	...
	Notificationsdb, err = sql.Open("sqlite3", "./DataBase/Notifications.db")
	if err != nil {
		log.Fatal(err)
	}


	NotificationssTableQuery := `CREATE TABLE IF NOT EXISTS notifications (
		creator_user_id INTEGER,
		follower_user_name TEXT,
		follower_profile TEXT,
		action TEXT,
		post_image TEXT,
		comment TEXT,
		FOREIGN KEY (creator_user_id) REFERENCES accounts(id));`

	_, err = Notificationsdb.Exec(NotificationssTableQuery)
	if err != nil {
		log.Fatal(err)
	}


	newNotificationssTableQuery := `CREATE TABLE IF NOT EXISTS newNotifications (
		user_id INTEGER,
		num_notifications INTEGER
	);`

	_, err = Notificationsdb.Exec(newNotificationssTableQuery)
	if err != nil {
		log.Fatal(err)
	}
