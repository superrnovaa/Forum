...
func CommentHandler(w http.ResponseWriter, r *http.Request) {
	...
	InsertNotification(comment.UserName, profileImage, postId , "Comment", comment.Text)
...
func GetCommentedPosts(id int) []string {
	var CommentedPostsId []string

	// Query the database for post data
	rows, err := Commentsdb.Query("SELECT post_id FROM comments WHERE user_id = ?", id)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		CommentedPostsId = append(CommentedPostsId, strconv.Itoa(PostId))
	}
	...
	return CommentedPostsId

}


func GetCreatedComments(id int) []string {
	var CommentsId []string

	// Query the database for post data
	rows, err := Commentsdb.Query("SELECT comment_id FROM comments WHERE user_id = ?", id)
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
